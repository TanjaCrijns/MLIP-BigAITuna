...
from . import patchsampler as sampler
...
class PatchGenerator(object):
	...
	def __init__(self, label_list, main_cache_size, read_size, patch_shape, label_shape, buffer_mode, augmentation_ratio, augmentation_list, sampling_mode, multi_threaded): 
		'''
		'''
	...
	def _init_sampler(patch_source, label_list, augmentation_ratio, augmentation_list, multi_threaded):
		'''
		'''
		...
		if self._multi_threaded:
			...
			self._patch_sampler = sampler.PatchSampler(patch_source, self._patch_shape, self._label_shape, self._label_list, self._augmentation_ratio, augmentation_list, sampling_mode)
	...
	def _load(self, batch_size, buffer):
		...
		patches, labels = self._patch_sampler.gen_batch(self._augmentation_ratio)
